  ...
  _createClass(Client, [{
    ...
    value: function listObjectsV2(bucketName, prefix, recursive) {
      ...
      readStream._read = function () {
        ...
        if (ended) return readStream.push(null);
        // if there are no objects to push do query for the next batch of objects
        _this12.listObjectsV2Query(bucketName, prefix, continuationToken, delimiter, 1000).on('error', function (e) {
          return readStream.emit('error', e);
        }).on('data', function (result) {
          if (result.isTruncated) {
    ...
    value: function presignedGetObject(bucketName, objectName, expires, respHeaders, cb) {
      ...
      this.getBucketRegion(bucketName, function (e, region) {
        ...
        // callback on presign failure.
        var url;
        var reqOptions = _this14.getRequestOptions({ method: method,
          region: region,
          bucketName: bucketName,
          objectName: objectName,
          query: query });
        try {
          url = (0, _signingJs.presignSignatureV4)(reqOptions, _this14.accessKey, _this14.secretKey, region, requestDate, expires);
        } catch (pe) {
          return cb(pe);
        }
        cb(null, url);
    ...
    value: function presignedPostPolicy(postPolicy, cb) {
      ...
      this.getBucketRegion(postPolicy.formData.bucket, function (e, region) {
        ...
        postPolicy.policy.conditions.push(["eq", "$x-amz-credential", _this15.accessKey + "/" + (0, _helpersJs.getScope)(region, date)]);
        postPolicy.formData['x-amz-credential'] = _this15.accessKey + "/" + (0, _helpersJs.getScope)(region, date);

        var policyBase64 = new Buffer(JSON.stringify(postPolicy.policy)).toString('base64');

        postPolicy.formData.policy = policyBase64;

        var signature = (0, _signingJs.postPresignSignatureV4)(region, date, _this15.secretKey, policyBase64);

        postPolicy.formData['x-amz-signature'] = signature;
        var opts = {};
        opts.region = region;
        opts.bucketName = postPolicy.formData.bucket;
        var reqOptions = _this15.getRequestOptions(opts);
        var portStr = _this15.port == 80 || _this15.port === 443 ? '' : ':' + _this15.port.toString();
    ...
    value: function listParts(bucketName, objectName, uploadId, cb) {
      ...
      var listNext = function listNext(marker) {
        _this16.listPartsQuery(bucketName, objectName, uploadId, marker, function (e, result) {
          if (e) {
            cb(e);
            return;
          }
    ...
    value: function findUploadId(bucketName, objectName, cb) {
      ...
      var listNext = function listNext(keyMarker, uploadIdMarker) {
        _this17.listIncompleteUploadsQuery(bucketName, objectName, keyMarker, uploadIdMarker, '').on('error', function (e) {
          return cb(e);
        }).on('data', function (result) {
    ...
    value: function chunkUploader(bucketName, objectName, contentType, uploadId, partsArray, multipartSize) {
      ...
      return _through22['default'].obj(function (chunk, enc, cb) {
        if (chunk.length > _this18.minimumPartSize) {
          return cb(new Error('chunk length cannot be more than ' + _this18.minimumPartSize));
        }
        ...
        if (!sha256 && _this18.enableSHA256) sha256 = _crypto2['default'].createHash('sha256');
        ...
        md5.update(chunk);
        if (_this18.enableSHA256) sha256.update(chunk);

        var done = false;
        if (aggregatedSize === multipartSize) done = true;
        // This is the last chunk of the stream.
        if (aggregatedSize < multipartSize && chunk.length < _this18.minimumPartSize) done = true;
        ...
        var sha256sum = '';
        if (_this18.enableSHA256) sha256sum = sha256.digest('hex');
        var md5sumBase64 = new Buffer(md5sumHex, 'hex').toString('base64');
        var multipart = true;
        var uploader = _this18.getUploader(bucketName, objectName, contentType, multipart);
    ...
    value: function getUploader(bucketName, objectName, contentType, multipart) {
      ...
      var upload = function upload(query, stream, length, sha256sum, md5sum, cb) {
        ...
        _this19.makeRequestStream({ method: method, bucketName: bucketName, objectName: objectName, query: query, headers: headers }, stream, sha256sum, 200, '', function (e, response) {
          if (e) return cb(e);
          var etag = response.headers.etag;
          if (etag) {
            etag = etag.replace(/^\"/, '').replace(/\"$/, '');
    ...
    value: function listenBucketNotification(bucketName, notificationARN) {
      if (!(0, _helpersJs.isValidBucketName)(bucketName)) {
        throw new errors.InvalidBucketNameError('Invalid bucket name: ' + bucketName);
      }
      ...
      var listener = new _notification.NotificationPoller(this, bucketName, notificationARN);
      listener.start();
